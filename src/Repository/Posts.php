<?php
/**
 * Posts repository for WP Post Types.
 *
...
use CodeAtlantic\Plugin\Model\Post;
use Exception;
 ...
 */
abstract class Posts implements \CodeAtlantic\Plugin\Interfaces\Repository {
	...
	public function get_item( $id ) {
		if ( ! $this->has_item( $id ) ) {
			throw new InvalidArgumentException( sprintf( __( 'No %1$s found with id %2$d.', 'default' ), $this->get_post_type(), $id ) );
		}
	...
	public function get_item_by( $field, $value ) {
		...
		if ( ! $id || ! $this->has_item( $id ) ) {
			throw new InvalidArgumentException( sprintf( __( 'No user found with %1$s %2$s.', 'default' ), $field, $value ) );
		}
